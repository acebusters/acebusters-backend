...
sinon.stub(contract.getTables, 'call').yields(null, [tableAddr]);
...
describe('Oracle pay', () => {
  it('should reject receipt with unknown hand Ids.', (done) => {
    ...
    const blind = new Receipt(tableAddr).bet(2, babz(50)).sign(P1_PRIV);
  ...
  it('should prevent small blind from player not in lineup.', (done) => {
    ...
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);

    const blind = new Receipt(tableAddr).bet(1, babz(50)).sign(P1_PRIV);
    oracle.pay(tableAddr, blind).catch((err) => {
  ...
  it('should prevent SB from leaving player.', (done) => {
    ...
    const blind = new Receipt(tableAddr).bet(3, babz(50)).sign(P2_PRIV);
    oracle.pay(tableAddr, blind).catch((err) => {
  ...
  it('should prevent BB from leaving player.', (done) => {
    ...
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P3_ADDR,
        last: new Receipt(tableAddr).bet(3, babz(50)).sign(P3_PRIV),
      }],
    }] });
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);
  ...
  it('should allow to play big blind with 3+ players.', (done) => {
    ...
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P2_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(50)).sign(P2_PRIV),
      }, {
        address: P3_ADDR,
      }],
      deck,
    ...
    const bet3 = new Receipt(tableAddr).bet(1, babz(100)).sign(P3_PRIV);
  ...
  it('should keed state dealing while 0 receipts submitted.', (done) => {
    ...
    sinon.stub(dynamo, 'query').yields(null, []).onFirstCall().yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P3_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(100)).sign(P3_PRIV),
      }, {
        address: P4_ADDR,
      }],
      deck,
  ...
  it('should set state preflop after last 0 receipts.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, []).onFirstCall().yields(null, { Items: [{
      ...
      lineup: [{
        address: P1_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(50)).sign(P1_PRIV),
      }, {
        address: EMPTY_ADDR,
      }, {
        address: P3_ADDR,
  ...
  it('should set state preflop after last 0 receipts and post small blind for sitout at tournament.', (done) => {
    ...
    sinon.stub(contract.getLineup, 'call').yields(null, [bn0, [P1_ADDR, P2_ADDR, P3_ADDR, P4_ADDR], [new BigNumber(BALANCE), new BigNumber(BALANCE), new BigNumber(BALANCE), new BigNumber(BALANCE)], [0, 0]]);
    sinon.stub(dynamo, 'updateItem').yields(null, {});
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc, 0, null, '', null, P4_PRIV);
  ...
  it('should prevent big blind from not in lineup.', (done) => {
    ...
    const blind = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
    oracle.pay(tableAddr, blind).catch((err) => {
      expect(err.message).to.contain('Forbidden');
      done();
    }).catch(done);
  ...
  it('should keep preflop until BB checked.', (done) => {
    ...
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        address: P1_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(100)).sign(P1_PRIV),
      }, {
        address: P2_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(50)).sign(P2_PRIV),
      }, {
    ...
    const bet = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
  ...
  it('should proceed to flop when BB gave checkPre.', (done) => {
    ...
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P2_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV),
      }],
      state: 'preflop',
      dealer: 0,
    }] });
    ...
    oracle.pay(tableAddr, pfCheck).then(() => {
      ...
      expect(dynamo.updateItem).calledWith(sinon.match.has('ExpressionAttributeValues', sinon.match.has(':m', babz(100).toString())));
      expect(dynamo.updateItem).calledWith(sinon.match.has('UpdateExpression', 'set #hand_state = :s, changed = :c, preMaxBet = :m, lineup[1] = :seat1'));
      done();
    }).catch(done);
  });
  ...
  it('should prevent betting more than balance in same hand.', (done) => {
    ...
    const tooMuchBet = new Receipt(tableAddr).bet(1, babz(200)).sign(P2_PRIV);
  ...
  it('should allow to fold.', (done) => {
    ...
    const fold = new Receipt(tableAddr).fold(1, babz(50)).sign(P2_PRIV);
    oracle.pay(tableAddr, fold).then((rsp) => {
      ...
      expect(dynamo.updateItem).calledWith(sinon.match.has('ExpressionAttributeValues', sinon.match.has(':seat1', seat)));
      done();
    }).catch(done);
  });
  ...
  it('should allow to go all in.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P3_ADDR,
        last: new Receipt(tableAddr).bet(1, babz(50)).sign(P3_PRIV),
      }],
    }] });
    sinon.stub(dynamo, 'updateItem').yields(null, {});
    sinon.stub(contract.getLineup, 'call').yields(null, [bn0, [P1_ADDR, P2_ADDR, P2_ADDR], [babz(1000), babz(100), babz(1000)], [bn0, bn0, bn0]]);
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);

    const allin = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
  ...
  it('should allow to call-in heads up and go to showdown.', (done) => {
    ...
    sinon.stub(contract.getLineup, 'call').yields(null, [
      ...
      [babz(1000), bn0, babz(100), babz(1000)],
      [bn0, bn0, bn0, bn0],
    ]);
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);
  ...
  it('should advance to showdown when last active player calls all-in.', (done) => {
    ...
    sinon.stub(contract.getLineup, 'call').yields(null, [
      ...
      [bn0, babz(2000), babz(2000), babz(1000)],
      [bn0, bn0, bn0, bn0]]);
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);
  ...
  it('should allow to call-in and proceed to showdown.', (done) => {
    ...
    sinon.stub(contract.getLineup, 'call').yields(null, [
      ...
      [babz(2000), babz(1000), babz(0), babz(2000)],
      [bn0, bn0, bn0, bn0],
    ]);
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);
  ...
  it('should allow multi-way all-in.', (done) => {
    ...
    sinon.stub(contract.getLineup, 'call').yields(null, [
      new BigNumber(2),
      [P1_ADDR, P2_ADDR, EMPTY_ADDR, P3_ADDR],
      [babz(800), babz(1300), babz(0), babz(1400)],
      [bn0, bn0, bn0, bn0],
    ]);
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);
  ...
  it('should advance to dealing when sit-out player calls sit-in and second player is waiting.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      handId: 13,
      dealer: 0,
      sb: '50000000000000',
      ...
      lineup: [{
        address: P1_ADDR,
        last: new Receipt(tableAddr).sitOut(13, babz(3)).sign(P1_PRIV),
        sitout: 15,
      }, {
        address: P2_ADDR,
        last: new Receipt(tableAddr).bet(13, babz(50)).sign(P2_PRIV),
    ...
    sinon.stub(dynamo, 'updateItem').yields(null, {});
    sinon.stub(contract.getLineup, 'call').yields(null, [new BigNumber(12), [P1_ADDR, P2_ADDR], [new BigNumber(BALANCE), new BigNumber(BALANCE), new BigNumber(BALANCE)], [0, 0]]);
    const oracle = new Oracle(new Db(dynamo), new TableContract(EMPTY_ADDR, web3), rc);

    const sitout = new Receipt(tableAddr).sitOut(13, babz(4)).sign(P1_PRIV);
    oracle.pay(tableAddr, sitout).then((rsp) => {
      expect(rsp).to.eql({});
      expect(dynamo.updateItem).calledWith(sinon.match.has('ExpressionAttributeValues', sinon.match.has(':s', 'dealing')));
  ...
  it('should finish hand when there is no active players and only one all-in player.', (done) => {
    ...
    oracle.pay(tableAddr, fold).then((rsp) => {
      ...
      const trueIsh = sinon.match((value) => {
        const l = [lineup[0], value.ExpressionAttributeValues[':seat1']];
  ...
  it('should switch to flop after fold when remaining pl. are even', (done) => {
    ...
    const fold3 = new Receipt(tableAddr).fold(1, babz(100)).sign(P3_PRIV);
  ...
  it('should prevent bet after fold.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P2_ADDR,
        last: new Receipt(tableAddr).fold(1, babz(100)).sign(P2_PRIV),
      }],
    }] });
    const oracle = new Oracle(new Db(dynamo), null, rc);
  ...
  it('should prevent playing lower bet.', (done) => {
    ...
    sinon.stub(dynamo, 'query').yields(null, []).onFirstCall().yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P2_ADDR,
        last: new Receipt(tableAddr).bet(2, babz(100)).sign(P2_PRIV),
      }],
      state: 'turn',
      deck,
      dealer: 0,
  ...
  it('should keep hand state if game ends.', (done) => {
    const bet1 = new Receipt(tableAddr).bet(1, babz(50)).sign(P1_PRIV);
    const bet2 = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
    ...
    const fold = new Receipt(tableAddr).fold(1, babz(50)).sign(P1_PRIV);
...
describe('Oracle info', () => {
  ...
  it('should allow to get showdown info.', (done) => {
    const show1 = new Receipt(tableAddr).show(0, babz(50)).sign(P1_PRIV);
    const muck2 = new Receipt(tableAddr).fold(0, babz(50)).sign(P2_PRIV);
    const lineup = [
...
describe('Oracle show', () => {
  ...
  it('should prevent bet in showdown', (done) => {
    const bet1 = new Receipt(tableAddr).bet(1, babz(100)).sign(P1_PRIV);
    const bet2 = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
    const lineup = [
      { address: P1_ADDR, last: bet1 },
      { address: P2_ADDR, last: bet2 },
  ...
  it('should prevent show by timedout player.', (done) => {
    const bet1 = new Receipt(tableAddr).bet(1, babz(100)).sign(P1_PRIV);
    const bet2 = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
    const bet3 = new Receipt(tableAddr).bet(1, babz(50)).sign(P3_PRIV);
  ...
  it('should prevent show by folded player.', (done) => {
    const bet1 = new Receipt(tableAddr).bet(1, babz(100)).sign(P1_PRIV);
    const bet2 = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
    const fold = new Receipt(tableAddr).fold(1, babz(50)).sign(P3_PRIV);
...
describe('Oracle leave', () => {
  ...
  it('should allow to leave in next hand.', (done) => {
    ...
    const lineup = [{
      address: P1_ADDR,
      last: new Receipt(tableAddr).bet(1, babz(2)).sign(P1_PRIV),
    }, {
      address: P2_ADDR,
      last: new Receipt(tableAddr).bet(1, babz(2)).sign(P2_PRIV),
    }];
    ...
    const oracle = new Oracle(
      new Db(dynamo),
      new TableContract(EMPTY_ADDR, web3),
      rc,
      timeoutPeriod,
    );
...
describe('Oracle netting', () => {
  it('should allow to deliver netting.', (done) => {
    ...
    const nettingSig = '0x1c306f6bc2348440582ca694d4998b082d3b77ad25b62fcf2f22e526a14e50ecf45bdb61d92d77bce6b5c7bce2800ddda525af1622af6b3d6f918993431fff1855';
...
describe('Oracle timing', () => {
  ...
  it('should allow to put player into sitout.', (done) => {
    const bet1 = new Receipt(tableAddr).bet(1, babz(100)).sign(P2_PRIV);
    ...
    oracle.timeout(tableAddr).then(() => {
      expect(dynamo.updateItem).calledWith(sinon.match.has('ExpressionAttributeValues', sinon.match.has(':seat1', {
        address: P2_ADDR,
        last: bet1,
  ...
  it('should handle sitout on hand state complete.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P2_ADDR,
        last: new Receipt(tableAddr).fold(1, babz(50)).sign(P2_PRIV),
      }],
    }] });
  ...
  it('should handle sitout on waiting state.', (done) => {
    ...
    const oracle = new Oracle(new Db(dynamo), null, rc);
    oracle.timeout(tableAddr).then(() => {
      expect(dynamo.updateItem).calledWith(sinon.match.has('ExpressionAttributeValues', sinon.match.has(':seat0', {
        address: P1_ADDR,
        sitout: sinon.match.number,
      })));
      done();
...
describe('Oracle lineup', () => {
  it('should handle Table leave.', (done) => {
    ...
    const oracle = new Oracle(new Db(dynamo),
      new TableContract(EMPTY_ADDR, web3), rc, null, null, null, logger);
