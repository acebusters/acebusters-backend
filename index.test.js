...
describe('Oracle pay', () => {
  ...
  it('should allow to call-in and proceed to showdown.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        ...
        address: P3_ADDR,
        last: new EWT(ABI_FOLD).fold(3, 800).sign(P3_KEY),
      }],
    }] });
    sinon.stub(dynamo, 'updateItem').yields(null, {});
    sinon.stub(contract.getLineup, 'call').yields(null, [
  ...
  it('should allow multi-way all-in.', (done) => {
    sinon.stub(dynamo, 'query').yields(null, { Items: [{
      ...
      lineup: [{
        address: P1_ADDR,
        last: new EWT(ABI_BET).bet(3, 800).sign(P1_KEY),
        sitout: 'allin',
      }, {
        address: P2_ADDR,
        last: new EWT(ABI_BET).bet(3, 800).sign(P2_KEY),
        ...
        address: P3_ADDR,
        last: new EWT(ABI_BET).bet(3, 1400).sign(P3_KEY),
        sitout: 'allin',
      }],
    }] });
    sinon.stub(dynamo, 'updateItem').yields(null, {});
    ...
    const call = new EWT(ABI_BET).bet(3, 1300).sign(P2_KEY);
    oracle.pay(tableAddr, call).then((rsp) => {
      expect(rsp).to.eql({});
      const seat = {
        address: P2_ADDR,
  ...
  it('should allow to sitout in state preflop heads up.', (done) => {
    ...
    const sitout = new EWT(ABI_SIT_OUT).sitOut(3, 50).sign(P1_KEY);
    oracle.pay(tableAddr, sitout).then((rsp) => {
      expect(rsp).to.eql({});
      const seat = {
        address: P1_ADDR,
