...
exports.handler = function(event, context, callback) {
  ...
  if (event.Records && event.Records instanceof Array) {
    ...
    var provider = new Provider(web3Provider, process.env.ACCOUNT_ADDR, TABLE_ABI);

    var requests = [];
    var worker = new StreamWorker(new Table(provider), new Db(dynamo), process.env.ORACLE_PRIV);
