...
TableContract.prototype.net = function(tableAddr) {
  ...
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.net.sendTransaction({from: self.senderAddr, gas: 2600000}, function(err, val){
      if (err) {
        console.log(JSON.stringify(err));
        reject(JSON.stringify(err));
        return;
...
TableContract.prototype.submitDists = function(tableAddr, distsHex, sigsHex) {
  ...
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.submitDists.sendTransaction(distsHex, sigsHex, {from: self.senderAddr, gas: 1900000}, function(err, val){
      if (err) {
        console.log(JSON.stringify(err));
        reject(JSON.stringify(err));
        return;
...
TableContract.prototype.submitBets = function(tableAddr, betsHex, sigsHex) {
  ...
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.submitBets.sendTransaction(betsHex, sigsHex, {from: self.senderAddr, gas: 1900000}, function(err, val){
      if (err) {
        console.log(JSON.stringify(err));
        reject(JSON.stringify(err));
        return;
...
TableContract.prototype.payout = function(tableAddr, signerAddr) {
  ...
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.payoutFrom.sendTransaction(signerAddr, {from: self.senderAddr, gas: 200000}, function(err, val){
      if (err) {
        console.log(JSON.stringify(err));
        reject(JSON.stringify(err));
        return;
...
TableContract.prototype.getSmallBlind = function(tableAddr) {
  ...
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.smallBlind.call(function(err, val){
      if (err) {
        console.log(JSON.stringify(err));
        reject(err);
        return;
...
TableContract.prototype.getLineup = function(tableAddr) {
  ...
  return new Promise(function (fulfill, reject) {
    contract.getLineup.call(function(err, lineup){
      if (!err && (!lineup || lineup.length < 4)) {
        err = 'lineup response invalid.';
      }
      if (err) {
        console.log(JSON.stringify(err));
        reject(err);
