...
TableContract.prototype.getLastHandNetted = function(tableAddr) {
  var self = this;
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.lastHandNetted.call(function(err, val){
      if (err) {
        reject(err);
      }
...
TableContract.prototype.getLastNettingRequestHandId = function(tableAddr) {
  var self = this;
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.lastNettingRequestHandId.call(function(err, val){
      if (err) {
        reject(err);
      }
      fulfill(val.toNumber());
...
TableContract.prototype.getLastNettingRequestTime = function(tableAddr) {
  var self = this;
  var contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
  return new Promise(function (fulfill, reject) {
    contract.lastNettingRequestTime.call(function(err, val){
      if (err) {
        reject(err);
      }
      fulfill(val.toNumber());
