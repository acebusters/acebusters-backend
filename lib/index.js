...
TableManager.prototype.pay = function(tableAddr, ewt) {
  var self = this, hand, prevHand, dist, deck, prevReceipt, pos = -1,
  ...
  return this.db.getHand(tableAddr, handId).then(function(_hand) {
    ...
    if (hand.distribution !== undefined)
      return Promise.reject('Bad Request: hand ' + handId + ' has distribution already.');
    ...
    if (receipt.abi[0].name == 'fold' &&
      ...
      dist = new EWT(ABI_DIST).distribution(handId, 0, dists).sign(RAKE_KEY);
...
TableManager.prototype.updateState = function(tableAddr, handId, hand, max, ewt, pos) {
    ...
    //update db
    return this.db.updateHand(tableAddr, handId, hand.lineup, hand.handState, pos);
}
...
TableManager.prototype.leave = function(tableAddr, ewt) {
  ...
  return this.db.getLastHand(tableAddr).then(function(_hand) {
    ...
    hand.lineup[pos].lastHand = receipt.values[1];
    return self.db.updateHand(tableAddr, handId, hand.lineup, hand.handState, pos);
  }).then(function() {
