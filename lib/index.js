...
TableManager.prototype.pay = function(tableAddr, ewt) {
  ...
  return this.db.getLastHand(tableAddr).then(function(_hand) {
    ...
    if (receipt.abi[0].name == 'fold' &&
      ...
      var dists = [];
      dists.push(EWT.concat(self.oracleAddr, Math.round( pot * RAKE)).toString('hex'));
      dists.push(EWT.concat(hand.lineup[winPos].address, Math.round(pot - (pot * RAKE))).toString('hex'));
      dist = new EWT(ABI_DIST).distribution(hand.handId, 0, dists).sign(self.oraclePriv);
      hand.lineup[pos].last = ewt;
      return self.db.updateHandDist(tableAddr, hand.handId, dist, hand.lineup[pos], pos);
    }
...
TableManager.prototype.show = function(tableAddr, ewt, cards) {
  ...
  return this.db.getHand(tableAddr, handId).then(function(_hand) {
    ...
    //distribute pots
    var dists = [];
    dists.push(EWT.concat(self.oracleAddr, Math.round(pots[0] * RAKE)).toString('hex'));
    for (i = 0; i < hand.lineup.length; i++) {
    ...
    dist = new EWT(ABI_DIST).distribution(handId, claimId, dists).sign(self.oraclePriv);
    return self.db.updateHandDist(tableAddr, handId, dist, hand.lineup[pos], pos);
  }).then(function() {
...
TableManager.prototype.leave = function(tableAddr, ewt) {
  ...
  return this.db.getLastHand(tableAddr).then(function(_hand) {
    ...
    // sign leave receipt
    const priv = new Buffer(self.oraclePriv.replace('0x', ''), 'hex');
