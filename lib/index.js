...
TableManager.prototype.timeout = function(tableAddr) {
  ...
  return this.db.getLastHand(tableAddr).then(function(_hand) {
    hand = _hand;
    if (hand.state === 'waiting') {
      return Promise.reject('Bad Request: hand ' + hand.handId + ' waiting.');
    }
    if (hand.state === 'waiting' || self.helper.checkForNextHand(hand)) {
      return Promise.reject('Bad Request: hand ' + hand.handId + ' completed.');
    }
