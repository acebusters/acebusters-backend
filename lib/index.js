...
EventWorker.prototype.calcDistribution = function(tableAddr, hand) {
  ...
  // create pots
  var i, j, pots = [], players = [];
  for (i = 0; i < hand.lineup.length; i++) {
    const last = (hand.lineup[i].last) ? EWT.parse(hand.lineup[i].last) : null;
    if (last) {
      const player = {
        pos: i,
      ...
      if (hand.state === 'showdown') {
        ...
        const isActive = self.helper.isActivePlayer(hand.lineup, i);
        if (isActive || hand.lineup[i].sitout === 'allin') {
          if (!contains.call(pots, last.values[1])) {
            pots.push(last.values[1]);
          }
          player.active = true;
  ...
  for (i = 0; i < evals.length; i++) {
    ...
    const winners = Solver.Hand.winners(hands);
    for (j = 0; j < winners.length; j++) {
      const pos = evals[i].chal[hands.indexOf(winners[j])];
...
EventWorker.prototype.putNextHand = function(tableAddr) {
  ...
  return Promise.all([hand, table, sb]).then(function(rsp) {
    ...
    lineup = rsp[1].lineup;
    smallBlind = rsp[2];
    var distProm = Promise.resolve(prevHand.distribution);
