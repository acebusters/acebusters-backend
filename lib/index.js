...
EventWorker.prototype.process = function(msg) {
  ...
  if (msgType === 'TableLeave') {
    tasks.push(this.table.leave(msgBody.tableAddr, msgBody.leaveReceipt).catch(err));
  }

  // handle HandComplete event:
  if (msgType === 'HandComplete') {
    if (!msgBody.handId || msgBody.handId === 0) {
      // table got deployed
      tasks.push(this.putFirstHand(msg.Subject.split('::')[1]));
    } else {
      tasks.push(this.putNextHand(msgBody.tableAddr, msgBody.handId).catch(err));
    }
  }
...
EventWorker.prototype.addPlayer = function(tableAddr) {
  ...
  return Promise.all([lup, ddp]).then(function(responses) {
    ...
    return self.db.updateSeat(tableAddr, hand.handId, hand.lineup[joinPos], joinPos, changed);
  });
}
...
EventWorker.prototype.putNextHand = function(tableAddr, prevHandId) {
  var self = this, prevHand;
  return self.db.getLastHand(tableAddr).then(function(_prevHand) {
    prevHand = _prevHand;
    if (prevHand.handId !== prevHandId)
      return Promise.reject('hand ' + prevHandId + ' not last hand.');
    ...
    const deck = shuffle();
    const changed = Math.floor(Date.now() / 1000);
    return self.db.putHand(tableAddr, prevHandId + 1, lineup, newDealer, deck, changed);
  });
}
