...
TableManager.prototype.pay = function(tableAddr, ewt) {
  var self = this, hand, prevHand, deck, prevReceipt, pos = -1,
  ...
  return this.db.getHand(tableAddr, handId).then(function(_hand) {
    ...
    if (hand.handState != 'dealing' && receipt.abi[0].name == 'bet' && receipt.values[1] < max.amount)
    ...
    if (hand.lineup[pos].last) {
      prevReceipt = EWT.parse(hand.lineup[pos].last);
    ...
    if (hand.handState == 'dealing') {
      ...
      var turn = self.helper.isTurn(hand.lineup, hand.dealer, receipt.signer);
      if (turn) {
        var smallBlind = self.helper.getSmallBlind(hand.lineup, hand.dealer);
        var sb = EWT.parse(smallBlind);
        if (!self.helper.isBlind(receipt, sb.values[1] * 2))
          return Promise.reject('Bad Request: big blind not valid.');
      } else {
        //collect 0 receipts
        if (!self.helper.isBlind(receipt, 0))
          return Promise.reject('Bad Request: only 0 receipts allowed');
      }
    }

    if ( (prevReceipt && prevReceipt.values[1] < receipt.values[1]) || !prevReceipt ) {
      //calc bal
      return self.calcBalance(tableAddr, pos, receipt).then(self.updateState(tableAddr, handId, hand, max, ewt, pos));
    } else {
      return self.updateState(tableAddr, handId, hand, max, ewt, pos);
    }
  }, function(err) {
  //if no
...
TableManager.prototype.updateState = function(tableAddr, handId, hand, max, ewt, pos) {
    hand.lineup[pos].last = ewt;
    if (this.helper.allDone(hand.lineup, hand.dealer, hand.handState, max.amount)) {
    ...
    //update db
    return this.db.updateHand(tableAddr, handId, hand.lineup, hand.handState);
}
    ...
    return self.contract.getLineup(tableAddr).then(function(rsp) {
      ...
      for (var i = 0; i < hands.length; i ++) {
        if (hands[i].lineup[pos].last)
          amount -= EWT.parse(hands[i].lineup[pos].last).values[1];
...
TableManager.prototype.show = function(tableAddr, ewt, cards) {
  ...
  return this.db.getHand(tableAddr, handId).then(function(_hand) {
    ...
    //distribute pots
    var dists = [];
    dists.push(EWT.concat(RAKE_ADDR, pots[0] * RAKE).toString('hex'));
    for (i = 0; i < hand.lineup.length; i++) {
      for (j = 0; j < winners.length; j++) {
        if (hands[i] == winners[j]) {
          dists.push(EWT.concat(hand.lineup[i].address, (pots[0] - (pots[0] * RAKE)) / (winners.length)).toString('hex'));
        }
      }
    }
    var claimId = 0;
