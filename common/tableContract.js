...
export default class TableContract extends Contract {
  allEvents(fromBlock, toBlock, address) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(address);
    const filter = contract.allEvents({
      fromBlock,
      toBlock,
    });
    return this.call(filter.get);
  }

  leave(tableAddr, leaveReceipt) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.sendContractTransaction(contract, 'leave', 200000, leaveReceipt);
  }

  toggleTable(tableAddr, activeReceipt) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.sendContractTransaction(contract, 'toggleActive', 200000, [activeReceipt]);
  }

  settle(tableAddr, sigs, newBalances) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.sendContractTransaction(contract, 'settle', 500000, [sigs, `0x${newBalances.substring(2, 66)}`, `0x${newBalances.substring(66, 130)}`]);
  }

  net(tableAddr) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.sendContractTransaction(contract, 'net', 2600000);
  }
  ...
  submit(tableAddr, receipts) {
    ...
    return this.call(contract.submit.call, receipts).then((writeCount) => {
      ...
      return this.sendContractTransaction(contract, 'submit', 1900000, [receipts]);
    });
  }
  ...
  getLastNettingRequestHandId(tableAddr) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.call(contract.lastNettingRequestHandId.call).then(n => n.toNumber());
  }

  getLastNettingRequestTime(tableAddr) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.call(contract.lastNettingRequestTime.call).then(n => n.toNumber());
  }
