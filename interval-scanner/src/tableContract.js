...
export default class TableContract extends Contract {
  ...
  getLineup(tableAddr) {
    const contract = this.web3.eth.contract(TABLE_ABI).at(tableAddr);
    return this.call(contract.getLineup.call).then(lineup => [
      ...lineup.slice(0, 3),
      lineup[3].map(n => n.toNumber()),
    ]);
  }
}
